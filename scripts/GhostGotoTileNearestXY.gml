...
ChangeDirection = false; // Eyes shouldn't reverse direction
...
repeat(3 + 1 * (lasttile == noone))
{
    ...
    if(othertile != noone) ds_priority_add(tlist,othertile,point_distance(othertile.x, othertile.y,targett.x,targett.y));
    //inc a by 90
    a+=90;
}
...
path_add_point(path,endt.x+6,endt.y+6,100);
